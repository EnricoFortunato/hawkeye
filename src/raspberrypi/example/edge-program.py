...
# Callback when the home topic receives a message
...
if __name__ == '__main__':
    ...
    # Subscribe
    print(f"Subscribing to topic {home_topic}")
    ...
    if counter == 0:
        ...
        print ("Sending {counter} message(s)")

    publish_count = 1
    while (publish_count <= counter) or (counter == 0):
        message = f"{message} [{publish_count}]"
        print(f"Publishing message to topic '{home_topic}': {message}")
    ...
    if counter != 0 and not received_all_event.is_set():
